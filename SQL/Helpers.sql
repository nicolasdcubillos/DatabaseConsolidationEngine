...
DECLARE @min BIGINT = CHANGE_TRACKING_MIN_VALID_VERSION(OBJECT_ID('dbo.MVTONIIF'));
IF @from < @min
BEGIN
...
SELECT
    ...
    s.*
FROM CHANGETABLE(CHANGES dbo.MVTONIIF, @from) AS ct
LEFT JOIN dbo.MVTONIIF AS s
  ON s.IDMVTO = ct.IDMVTO;
